    ...
    <>
      ...
      <Container className="py-5 mt-5">
        <div className="boxUserOrder">
          <img src={users.picture} alt={users.picture} className="imgUserOrder" />
          <div>
        ...
        {/* <div className="text-end">
          <BtnPrimary className="p3">Sudah dibaca</BtnPrimary>
        </div> */}
        <Row>
          <Col md={6} className="mb-3">
            ...
            <div className="scrollContent">
              {sellers.map((data) => {
                if (data.Order !== null) {
                  if (data.is_read === false && data.Order.Product.id_user === users.id) {
                    axios
                      .patch(`https://tokoku-api-2.herokuapp.com/api/v1/notification/${data.id}`, null, {
                        headers: {
                          Authorization: "Bearer " + localStorage.getItem("token"),
                        },
                      })
                  ...
                  return (
                    <div className="productOrder">
                      <div className="d-flex flex-row">
                        <div>
                          <img className="imgProductOrder" src={data.Order.Product.picture} alt={data.Order.Product.picture} />
                        </div>
                        <div>
                          <p className="textGray">Penawaran Produk Baru</p>
                          <h5 className="textContent">{data.Order.Product.name}</h5>
                          <h5 className="textContent">Rp {data.Order.Product.price.toLocaleString("id-ID")}</h5>
                        </div>
                      </div>
                      <div className="text-end">
                        <p className="textGray mb-2">{moment(data.createdAt).format("L")}</p>
                        <Link
                          style={{
                            textDecoration: "none",
                            color: "black",
                    ...
                    {data.status === "bid" ? (
                      <div className="productOrder" key={data.id}>
                        <div className="d-flex flex-row">
                          ...
                          <div>
                            <p className="textGray">Anda berhasil melakukan penawaran</p>
                            <h5 className="textContent">{data.Order.Product.name}</h5>
                            <h5 className="textContent">Rp {data.Order.Product.price.toLocaleString("id-ID")}</h5>
                            <h5 className="textContent">Ditawar Rp {data.Order.price.toLocaleString("id-ID")}</h5>
                          </div>
                          <div className="text-end">
                            <p className="textGray mb-2">{moment(data.createdAt).format("L")}</p>
                          </div>
                        ...
                        {data.status === "accepted" ? (
                          <div className="productOrder" key={data.id}>
                            <div className="d-flex flex-row">
                              ...
                              <div>
                                <p className="textGray">Penawaran anda disetujui</p>
                                <h5 className="textContent">{data.Order.Product.name}</h5>
                                <h5 className="textContent">Rp {data.Order.Product.price.toLocaleString("id-ID")}</h5>
                                <h5 className="textContent">Ditawar Rp {data.Order.price.toLocaleString("id-ID")}</h5>
                              </div>
                          ...
                          <div className="productOrder" key={data.id}>
                            <div className="d-flex flex-row">
                              ...
                              <div>
                                ...
                                <h5 className="textContent">Rp {data.Order.Product.price.toLocaleString("id-ID")}</h5>
                                <h5 className="textContent">Ditawar Rp {data.Order.price.toLocaleString("id-ID")}</h5>
                                <p className="textGray">cek history order untuk merubah penawaran.</p>
                              </div>
                            </div>
                            <div className="text-end">
                              <p className="textGray mb-2">{moment(data.createdAt).format("L")}</p>
